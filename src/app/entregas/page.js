...
export default function EntregasPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (!form.socio_id) {
      setFormError('Selecciona un socio')
      setSaving(false)
      return
    }
    if (!form.kg_bruto || parseFloat(form.kg_bruto) <= 0) {
      setFormError('Los kg brutos deben ser mayores que 0')
      setSaving(false)
      return
    }
    ...
    const payload = {
      ...
      fecha: form.fecha,
      kg_bruto: parseFloat(form.kg_bruto),
  ...
  function kgAceitePorEntrega(entrega) {
    if (!entrega.rendimiento) return '—'
    return ((entrega.kg_bruto * entrega.rendimiento) / 100).toLocaleString('es-ES', { maximumFractionDigits: 1 }) + ' kg'
  }

  const totalKg = entregas.reduce((sum, e) => sum + (e.kg_bruto || 0), 0)
  const totalAceite = entregas.reduce((sum, e) => {
    if (!e.rendimiento) return sum
    return sum + (e.kg_bruto * e.rendimiento) / 100
  }, 0)
        ...
        <div className="p-7">
          ...
          {showForm && (
            <div className="bg-white rounded-xl border border-gray-100 p-6 mb-6">
              <h2 className="text-sm font-bold text-gray-900 mb-5">Registrar nueva entrega</h2>
              <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <select
                    ...
                    required
                    className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#7ab648] bg-white"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    value={form.fecha}
                    onChange={handleChange}
                    className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#7ab648]"
                  />
                </div>

                <div>
                  <label className="block text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">
                    Kg bruto <span className="text-red-400">*</span>
                  </label>
                  <input
                    type="number"
                    name="kg_bruto"
                ...
                {formError && (
                  <div className="col-span-2 bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-2.5 rounded-lg">
                    {formError}
                  </div>
                )}
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-[#1a2e1a] rounded-xl p-5">
              ...
              <div className="text-2xl font-extrabold tracking-tight text-white">
                {(totalKg / 1000).toFixed(1)}<span className="text-sm font-medium ml-1 text-white/40">t</span>
              </div>
            </div>
            <div className="bg-white rounded-xl p-5 border border-gray-100">
              ...
              <div className="text-2xl font-extrabold tracking-tight text-gray-900">
                {(totalAceite / 1000).toFixed(2)}<span className="text-sm font-medium ml-1 text-gray-400">t</span>
              </div>
            </div>
            <div className="bg-white rounded-xl p-5 border border-gray-100">
          ...
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            ...
            {entregas.length === 0 ? (
              <div className="px-6 py-12 text-center text-gray-400 text-sm">
                Aún no hay entregas registradas en esta campaña.
              </div>
            ) : (
                ...
                <tbody>
                  {entregas.map((e, i) => (
                    <tr key={e.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                      ...
                      <td className="px-6 py-3 text-right text-[#4a7a1e] font-semibold tabular-nums">{kgAceitePorEntrega(e)}</td>
