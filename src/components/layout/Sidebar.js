    ...
    items: [
      {
        ...
        icon: <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.437L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />,
      },
      {
        href: '/admin',
...
export default function Sidebar() {
  ...
  return (
    <div className="flex flex-col h-screen w-60 flex-shrink-0 overflow-hidden"
      style={{ backgroundColor: '#0c1425', borderRight: '1px solid rgba(255,255,255,0.05)' }}>

      {/* Logo */}
      <div className="px-4 pt-5 pb-4">
        <div className="flex items-center gap-3 px-2">
          <div className="relative flex-shrink-0">
            <img src="/logo.png" alt="OlivaGest" className="w-8 h-8 rounded-xl" />
            <div className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-[#4ade80] border-2"
              style={{ borderColor: '#0c1425' }} />
          </div>
          <div>
            <p className="text-white font-bold text-sm leading-tight tracking-tight">OlivaGest</p>
            <p className="text-xs leading-tight font-medium" style={{ color: '#4ade80' }}>Gestión oleícola</p>
          </div>
        </div>
      </div>
              ...
              {grupo.items.map(item => {
                ...
                return (
                  ...
                  >
                    {/* Indicador activo */}
                    {isActive && (
                      <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-r-full bg-[#4ade80]" />
                    )}
      ...
      <div className="px-3 pb-4" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        <div className="pt-3">
          ...
          <p className="text-xs px-3 mt-2" style={{ color: 'rgba(255,255,255,0.15)' }}>OlivaGest v1.0</p>
        </div>
      </div>
    </div>
  )
